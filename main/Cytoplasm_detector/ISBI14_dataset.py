...
class TrainingDataset(data.Dataset):
    ...
    def __init__(self, data_dirs):
        ...
        """ Rank maps: generated from matlab for all training images in ISBI (855) """   
        self.rankmap_list = io.loadmat(data_dirs[2])
        self.rankmaps = self.rankmap_list['ranked_maps'] # matlab variable name for extracting from file
        self.rankmaps = np.squeeze(self.rankmaps)
        self.rankmaps = self.rankmaps[0:n] # use only X examples to test if the function is working properly
    ...
    def __getitem__(self, index):
        ...
        ------------------------
        """
        # Get the ith file from the list. 
        rankmap_i = self.rankmaps[index]
        train_rankmap_tensor = torch.from_numpy(rankmap_i).float().unsqueeze(0)
        
        return [train_img_tensor, train_target_tensor, train_rankmap_tensor]
